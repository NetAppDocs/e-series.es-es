...
summary: En los sistemas de almacenamiento EF50 y EF80, puedes reemplazar un cajón de unidades en una bandeja de expansión DE460C cuando la bandeja de unidades está en línea o fuera de línea. 
---
= Sustituir el cajón de unidades DE460C en los sistemas de almacenamiento EF50 y EF80
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
En los sistemas de almacenamiento EF50 y EF80, puedes reemplazar un cajón de unidades en una bandeja de expansión DE460C cuando la bandeja de unidades está en línea o fuera de línea.

.Acerca de esta tarea
Los pasos para sustituir un cajón de unidades con error en una bandeja de unidades DE460C dependen de si los volúmenes del cajón están protegidos por Drawer Loss Protection. Si todos los volúmenes del cajón de unidades están en pools de discos o grupos de volúmenes que tienen Drawer Loss Protection, puedes realizar este procedimiento en línea. De lo contrario, debes realizar este procedimiento fuera de línea, deteniendo toda la actividad de E/S del host y apagando todo el sistema de almacenamiento (el estante del controlador y todos los estantes de expansión) antes de sustituir un cajón de unidades.
...
** Acceso a la interfaz de línea de comandos (CLI) del sistema de almacenamiento. Si no tienes acceso a la CLI, puedes hacer una de las siguientes cosas:
+
...
NOTE: Si necesitas información sobre cómo sustituir una bandeja de expansión DE460C de la E-Series, consulta el  https://kb.netapp.com/on-prem/E-Series/Hardware-KBs/How_to_replace_an_E_Series_DE460c_controller_expansion_shelf["Base de conocimientos de NetApp"^].
...
Determina si puedes llevar a cabo el procedimiento de sustitución mientras la bandeja de unidades está en línea o si necesitas detener la actividad de E/S del host y apagar el sistema de almacenamiento (realizar el procedimiento fuera de línea).

Si vas a sustituir un cajón de una bandeja que cuenta con la función Drawer Loss Protection, no es necesario detener la actividad de E/S del host ni apagar el sistema de almacenamiento.

.Pasos
. Comprueba si la bandeja de unidades de destino está encendida.
+
** Si no hay alimentación eléctrica, no necesitas ejecutar el comando de la CLI para poner el cajón de destino en modo de servicio. Ve a <<Paso 2: Quitar las cadenas para cables>>.
** Si la alimentación está encendida, pasar al siguiente paso.


. Accede a la CLI y, a continuación, introduce el siguiente comando para poner el cajón de destino en modo de servicio:
+
[listing]
----
...
**  `array_username` es el nombre de usuario de la matriz de almacenamiento. Debes escribir el valor de  `array_username` entre comillas dobles ("").
...
** Si aparece un aviso indicando que no se ha podido completar este comando, debes detener la actividad de E/S del host antes de retirar el cajón. La advertencia aparece porque una o más unidades del cajón afectado se encuentran en pools o grupos de volúmenes que no cuentan con Drawer Loss Protection. Para evitar la pérdida de datos, debes completar los siguientes pasos para detener la actividad de E/S del host y apagar la bandeja del controlador y todas las bandejas de expansión.
...
. Si el sistema de almacenamiento forma parte de una relación de duplicación, detén todas las operaciones de E/S del host en el sistema de almacenamiento secundario.
+

CAUTION: *Posible pérdida de datos* -- Si continúas con este procedimiento mientras se están realizando operaciones de E/S, la aplicación host podría perder datos porque el sistema de almacenamiento no estará accesible.
...
. Apaga todo el sistema de almacenamiento si tienes bandejas de expansión *sin* la función _Drawer Loss Protection_:
+

NOTE: Si vas a sustituir un cajón en una estantería *con* _Protección contra pérdida de cajones_: NO es necesario apagar ninguna de las estanterías. Puedes realizar el procedimiento de sustitución mientras el cajón de la unidad está conectado, ya que el comando CLI Set Drawer Service Action Allowed Indicator se ha ejecutado correctamente.
...
.. Apaga los dos interruptores de encendido de cada una de las bandejas de expansión.
...
CAUTION: *Posible daño en el hardware* -- Si el bastidor de unidades está encendido, la cadena de cables permanece bajo tensión hasta que se desconecten ambos extremos. Para evitar que se produzca un cortocircuito en el equipo, no permitas que el conector de la cadena de cables desconectado entre en contacto con el chasis metálico si el otro extremo de la cadena de cables sigue conectado.

.Pasos
. Asegúrate de que el bastidor del controlador y las bandejas de expansión ya no presenten actividad de E/S y estén apagados, o de que hayas ejecutado el comando de la CLI  `Set Drawer Attention Indicator`.
. Desde la parte trasera de la bandeja de unidades de destino, retira la carcasa del ventilador de la derecha:
+
...
.. Usando el asa, saca el compartimento del ventilador de la bandeja de unidades y déjalo a un lado.
.. Si la bandeja está encendida, asegúrese de que el ventilador izquierdo vaya a su velocidad máxima.
+

CAUTION: *Posible daño en el equipo debido al sobrecalentamiento* -- Si el bastidor de unidades está encendido, no retires ambos ventiladores a la vez. De lo contrario, el equipo podría sobrecalentarse.
...
. Asegúrese de que:
+
** Se ha instalado una nueva bandeja para discos.
...
Vuelve a colocar las unidades en el orden correcto, vuelve a instalar la bandeja de unidades en la bandeja de expansión, vuelve a colocar el frontal, vuelve a conectar la alimentación (si es necesario), reconstruye manualmente las unidades (si es necesario) y luego devuelve la pieza con error a NetApp.
...
. Si has apagado el sistema de almacenamiento (el shelf del controlador y las bandejas de expansión) para sustituir un cajón de discos, vuelve a conectarlo a la corriente:
+
.. Enciende los dos interruptores de encendido de todas las bandejas de expansión.
...
. Si realizaste el procedimiento de sustitución de la bandeja de unidades en línea y alguna de las unidades de la bandeja de recambio es una unidad con error, debes reconstruir las unidades manualmente.
+
Utilizando SANtricity System Manager, completa el procedimiento link:https://docs.netapp.com/us-en/e-series-santricity/sm-hardware/reconstruct-drive-manually.html["Reconstruir una unidad manualmente"^].
