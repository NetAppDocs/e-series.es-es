...
---
= Sustituir un HIC averiado en un sistema de almacenamiento EF300 o EF600
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
Cuando sustituyes un HIC averiado, debes desconectar un controlador para poner el sistema en un estado sin redundancia, reemplazar el HIC y volver a poner el controlador en línea.


NOTE: No es necesario apagar las matrices de almacenamiento EF300 y EF600 al sustituir un HIC del mismo tipo de protocolo (por ejemplo, al sustituir un HIC de Fibre Channel por otro HIC de Fibre Channel).

.Antes de empezar
* Revisar link:hics-overview-supertask-concept.html["Requisitos para las actualizaciones de EF 300 o EF600 HIC"].
* Programa una ventana de mantenimiento con interrupción del servicio para este procedimiento. No puedes acceder a los datos de la matriz de almacenamiento hasta que completes correctamente este procedimiento. Como ambos controladores deben tener la misma configuración de HIC cuando están encendidos, tienes que desconectar uno de los controladores para poner el sistema en un estado de no redundancia cuando instales los HIC.
...
+
.. Selecciona el menú: Storage [Volumes]. En la página *All Volumes*, comprueba que los volúmenes estén distribuidos a sus propietarios preferidos. Selecciona el *ellipsis icon* > *Change ownership* para ver los propietarios de los volúmenes.
