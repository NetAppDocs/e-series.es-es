...
summary: Para el protocolo NVMe over Fibre Channel, se deben configurar los switches y determinar los identificadores de puerto de host. 
---
= Configura los switches NVMe over FC para VMware en E-Series
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Al planificar tu configuración de NVMe sobre FC, recuerda que el https://configmax.broadcom.com/guest["Guía de límites máximos de configuración de VMware"^] indica que el número máximo de puertos iniciadores NVMe FC admitidos por servidor es 2. Debes tener en cuenta este requisito para evitar configurar demasiados caminos.
...
Puedes generar e imprimir un PDF de esta página y, a continuación, utilizar la siguiente hoja de trabajo para anotar la información de configuración del almacenamiento NVMe sobre Fibre Channel. Necesitas esta información mientras realizas tareas de aprovisionamiento.

La ilustración muestra un host conectado a un almacenamiento E-Series en dos zonas. Una zona se indica con líneas rojas; la otra zona se indica con líneas azules. Cada zona contiene un puerto iniciador y la mitad de todos los puertos de destino. El número de puertos del controlador variará según el modelo del controlador y el número de tarjetas de interfaz del host (HIC). Puedes tener más puertos disponibles (hasta 12 por controlador). Imprime archivos PDF adicionales según lo necesites.

image::../media/port_identifiers_host_and_target_conf-vmw.gif[Identificadores de puertos NVMe sobre Fibre Channel]
...
== Paso 2: determina los puertos del host y los WWPN
...
. Ejecuta el siguiente comando y anota el NQN del host (lo necesitarás más adelante cuando configures las asignaciones de host):
+
[listing]
----
...
== Paso 3: Configura los conmutadores NVMe/FC
