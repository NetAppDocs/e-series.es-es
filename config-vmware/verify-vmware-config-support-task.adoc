...
** Cada fabricante de HBA dispone de métodos específicos para actualizar el código de arranque y el firmware. Algunos de estos métodos podrían incluir el uso de un complemento de vCenter o la instalación de un proveedor CIM en el host ESXi. Los complementos de vCenter se pueden utilizar para obtener información sobre el HBA específico del fabricante. Consulta la sección de soporte del sitio web del fabricante para obtener las instrucciones y el software necesarios para actualizar el código de arranque o el firmware del HBA. Consulta https://compatibilityguide.broadcom.com/search?program=io&persona=live&column=brandName&order=asc["_Guía de compatibilidad de VMware_"] o el sitio web del fabricante del HBA para obtener el código de arranque o el firmware correctos.
** Revisa las alertas y la información asociadas con la configuración seleccionada para cualquier sistema operativo específico, sistema operativo invitado, HBA u otros ajustes o información relacionada que pueda necesitar aplicarse a tu configuración.
...
NVMe over roce
 a| 
Controlador de Remote Direct Memory Access (RDMA), firmware y bootcode
