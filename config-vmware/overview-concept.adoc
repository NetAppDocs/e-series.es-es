...
---
= Supuestos de configuración express de VMware para E-Series
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Utiliza versiones de controlador y firmware SAS HBA tal como se indica en la http://mysupport.netapp.com/matrix["Herramienta de matriz de interoperabilidad de NetApp"^].
 ...
 a| 
Protocolo: NVMe sobre RoCE (RDMA sobre Ethernet convergente)
 a| 
* Estás utilizando conmutadores Ethernet compatibles con tráfico RDMA
* Has configurado los conmutadores Ethernet para una red sin pérdidas usando el control de flujo prioritario (PFC) basado en Differentiated Services Code Point (DSCP)
* Estás utilizando las versiones del controlador y del firmware de RDMA que figuran en http://mysupport.netapp.com/matrix["Herramienta de matriz de interoperabilidad de NetApp"^].




 a| 
Protocolo: NVMe sobre TCP
 a| 
* Estás utilizando conmutadores Ethernet compatibles con tráfico TCP
* Has configurado los conmutadores Ethernet según la recomendación del proveedor para TCP.
* Estás utilizando las versiones de controlador de red y firmware que figuran en http://mysupport.netapp.com/matrix["Herramienta de matriz de interoperabilidad de NetApp"^].


|===
Si estas suposiciones no se ajustan a tu instalación, o si deseas obtener más información conceptual de fondo, consulta el siguiente informe técnico: https://www.netapp.com/pdf.html?item=/media/17017-tr4789.pdf["Guía de configuración de VMware para la integración de iSCSI de SANtricity E-Series con ESXi 6.X."^]
