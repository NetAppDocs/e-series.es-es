...
---
= Mostrar los volúmenes visibles para un host Linux mediante NVMe sobre RoCE en E-Series
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Para ver información sobre cada ruta NVMe en un volumen E-Series, utilice `nvme netapp smdevices [-o <format>]` comando. El <format> de salida puede ser normal (el valor predeterminado si -o no se utiliza), columna o json.
...
----
.El futuro
Una vez que hayas consultado los volúmenes disponibles, link:nvme-roce-setup-failover-host-task.html["configurar la conmutación por error en el host"].
