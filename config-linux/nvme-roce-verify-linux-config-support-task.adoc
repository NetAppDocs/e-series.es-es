...
---
= Verifica la compatibilidad de la configuración de Linux con NVMe sobre RoCE en E-Series
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
CAUTION: *RIESGO DE PÉRDIDA DE DATOS.* Debes ejecutar NVMe sobre RoCE en un entorno sin pérdidas. Activa el control de flujo de extremo a extremo usando Global Pause Frames o Priority Flow Control (PFC) en tu red para eliminar el riesgo de pérdida de datos. PFC requiere que NVMe sobre RoCE se etiquete como tráfico de prioridad 3 con un valor DSCP de `106`.
...
.El futuro
Después de que verifiques la compatibilidad de la configuración con Linux y revises las restricciones, link:nvme-roce-configure-ip-address-dhcp-task.html["configurar direcciones IP mediante DHCP"].
