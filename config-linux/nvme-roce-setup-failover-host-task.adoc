...
SLES 12 utiliza Device Mapper Multipath (DMMP) para rutas múltiples para NVMe sobre RoCE. RHEL 8, RHEL 9, RHEL 10, SLES 15 y SLES 16 utilizan una conmutación por error NVMe nativa integrada. Dependiendo del sistema operativo que esté ejecutando, se requiere alguna configuración adicional de multipath para que funcione correctamente.
...
NOTE: Para SLES 15, SLES 16, RHEL 9 y RHEL 10, la multirruta NVMe nativa está habilitada de manera predeterminada y no se requiere ninguna configuración adicional.
