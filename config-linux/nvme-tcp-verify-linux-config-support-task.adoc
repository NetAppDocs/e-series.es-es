...
---
= Verifica la compatibilidad de la configuración de NVMe over TCP en Linux en E-Series
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
== Verifica las restricciones de NVMe sobre TCP

Antes de utilizar NVMe sobre TCP, consulta la  https://mysupport.netapp.com/matrix["Herramienta de matriz de interoperabilidad de NetApp"^] para revisar las últimas restricciones de controladores, hosts y recuperación.
...
.El futuro
Después de que verifiques la compatibilidad de la configuración con Linux y revises las restricciones, link:nvme-tcp-configure-ip-address-dhcp-task.html["configurar direcciones IP mediante DHCP"].
